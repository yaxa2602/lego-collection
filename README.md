...
- **Страница набора** (`/set/[номер]`) — фото, факты о наборе, ссылки на площадки (Avito, Ozon, eBay, Amazon, BrickLink) и на инструкцию по сборке с lego.com.
...
- Цен в приложении нет намеренно: Rebrickable их не отдаёт, а официальные цены (Brickset API) требуют отдельного ключа. Вместо оценок — ссылки-поиски по площадкам: Avito и Ozon (Россия), eBay и Amazon (зарубеж), BrickLink (маркетплейс фанатов). Там цены живые.

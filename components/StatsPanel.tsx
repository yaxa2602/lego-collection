...
import { totals, groupByFranchise, groupByYear, estimateRub } from "@/lib/stats";
    ...
    <>
      <section className="stats">
        <div className="card stats-card totals">
          ...
          <div><b>≈&nbsp;{rub.toLocaleString("ru-RU")}&nbsp;₽</b><span className="muted">оценка стоимости</span></div>
        </div>
        <Bars title="По франшизам" items={groupByFranchise(entries, themes)} />
        <Bars title="По годам" items={groupByYear(entries)} />
      </section>
      <p className="stats-note">Стоимость — грубая прикидка из расчёта ~10&nbsp;₽ за деталь, не реальная цена.</p>
    </>
  );
}
